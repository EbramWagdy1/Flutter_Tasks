...
import 'package:screen75/step.dart';
...
class _HomeState extends State<Home> {
    ...
    for (var step in steps) {
      if (!step.isExpanded) return false;
    }
    return true;
  }
  @override
  Widget build(BuildContext context) {
      bool allExpanded = areAllExpanded();
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
      ...
      body: Column(
        children: [
         ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1C1C14)),
            onPressed: () {
              setState(() {
                for (var step in steps) {
                  step.isExpanded = !allExpanded;
                }
              });
            },
